...
class RunControllerTest extends TestCase
{
    ...
    /**
     * Issue #66 review: judge() had no guard against re-judging a run that
     * is already status='judged' -- unlike the web JudgeController::judge()
     * this endpoint mirrors, which rejects it to avoid double-counting the
     * attempt in Score::updateScore().
     */
    public function test_judge_run_rejects_an_already_judged_run()
    {
        $contest = Contest::factory()->create();
        $accepted = Answer::factory()->create(['contest_id' => $contest->id]);
        $wrong = Answer::factory()->create(['contest_id' => $contest->id]);
        $run = Run::factory()->create([
            'contest_id' => $contest->id,
            'status' => 'judged',
