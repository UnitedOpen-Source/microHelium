...
| RF-F04-008 | P0 | Manter a instituição pela qual uma equipe compete em vínculo próprio (`users.organization_id`), distinto da governança do banco de problemas (`organization_memberships`). | Quem edita o acervo de uma instituição e compete por outra aparece com a instituição pela qual compete; equipe que nunca tocou o banco tem instituição; `php artisan affiliations:report` lista quem ficou sem e quem a derivação não pôde decidir. |
| RF-F04-009 | P1 | Publicar identidade externa da instituição (`icpc_id`, `formal_name`, `country`) e o `icpc_id` da equipe na Contest API. | Consumidor consegue casar equipe e instituição com cadastro externo; `formal_name` ausente cai no `name`. |
| RF-F04-010 | P1 | Exportar um pacote de resultados de prova **finalizada**, com identidade estável do evento e SHA-256 por arquivo. | `GET /api/frontend/contests/{id}/results-bundle` recusa prova não finalizada com 409 e motivo legível; duas exportações da mesma prova diferem apenas em `manifest.generated_at`; nenhum campo protegido pela `ProfilePrivacyPolicy` aparece em arquivo algum do pacote. |
...
| ENT-002b | organizations / Organization | Instituição pela qual uma equipe compete, e dona de itens do banco de problemas. | id, name, icpc_id, formal_name, country, archived_at | users (afiliação), organization_memberships (governança do acervo) |
| ENT-001b | contests (identidade) | Identidade do evento fora desta instalação (#271). | uuid (imutável, único), edition, phase | pacote de resultados |
