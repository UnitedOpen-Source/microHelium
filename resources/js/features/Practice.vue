...
import { protectDraft } from '../ui/draft.js';
...
const sourceBytes = computed(() => new TextEncoder().encode(source.value).length);
const dirty = computed(() => source.value !== submittedDraft.value && source.value.length > 0);
...
async function submit(event) {
    ...
    const result = await act(`${endpoint}/runs`, { language_id: language.value, source: source.value }, 'Solução recebida. Consulte o histórico para acompanhar o julgamento.', 'POST', event.target);
    if (result) submittedDraft.value = submittedSource;
}
