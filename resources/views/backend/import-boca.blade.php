...
@section('page-actions')<a href="{{ route('backend.problem-bank') }}" class="button-secondary">← Voltar ao banco de problemas</a>@endsection

@section('content')
<div class="max-w-4xl mx-auto space-y-6">

    <!-- Info Card -->
    <div class="bg-info-soft border border-info/25 rounded-lg p-4">
        <div class="flex gap-3">
            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-info flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
                <h3 class="font-semibold text-info">Sobre o formato BOCA</h3>
    ...
    <div class="grid gap-6 md:grid-cols-2">
        ...
        <div class="bg-card border border-border rounded-lg">
            ...
            <form action="/backend/import-boca/upload" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div id="dropZone" class="border-2 border-dashed border-input rounded-lg p-6 text-center focus-within:ring-2 focus-within:ring-primary">
                    <input type="file" name="boca_zip" id="fileInput" accept=".zip" class="sr-only" required onchange="updateFileName(this)" aria-describedby="zip-hint">
                    <label for="fileInput" class="cursor-pointer">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-muted-foreground/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <p id="zip-hint" class="text-sm text-muted-foreground mt-2">Selecione um pacote .zip por vez.</p>
                    </label>
                    <p role="status" id="fileName" class="mt-4 text-sm font-medium text-primary break-all hidden"></p>
                </div>
                <button type="submit" class="w-full mt-4 px-4 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary-hover transition-colors font-medium">
                    Importar Arquivo
                </button>
            </form>
        </div>
        ...
        <div class="bg-card border border-border rounded-lg">
            ...
            <div class="p-6">
                ...
                <form action="/backend/import-boca/github" method="POST">
                    @csrf
                    <button type="submit" class="w-full px-4 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary-hover transition-colors font-medium flex items-center justify-center gap-2">
